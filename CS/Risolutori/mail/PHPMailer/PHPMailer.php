...
class PHPMailer
{
     ...
     * Literal Constants
     */
    private static $TXT_HTML = 'text/html';
    private static $TXT_PLAN = 'text/plain';
    private static $SENDEMAIL = 'sendmail';
    private static $QMAIL = 'qmail';
    private static $REPLYTO = 'Reply-To';
    private static $INV_ADDR = 'invalid_address';
    ...
    {
        if ($isHtml) {
            $this->ContentType = static::$TXT_HTML;
        } else {
            $this->ContentType = static::$TXT_PLAN;
        }
    }
    ...
    public function isSendmail()
    {
        ...
        if (false === stripos($ini_sendmail_path, static::$SENDEMAIL)) {
            $this->Sendmail = '/usr/sbin/sendmail';
        } else {
            $this->Sendmail = $ini_sendmail_path;
        }
        $this->Mailer = static::$SENDEMAIL;
    }
    ...
    public function isQmail()
    {
        ...
        if (false === stripos($ini_sendmail_path, static::$QMAIL)) {
            $this->Sendmail = '/var/qmail/bin/qmail-inject';
        } else {
            $this->Sendmail = $ini_sendmail_path;
        }
        $this->Mailer = static::$QMAIL;
    }
    ...
    {
        return $this->addOrEnqueueAnAddress(static::$REPLYTO, $address, $name);
    }
    ...
    protected function addOrEnqueueAnAddress($kind, $address, $name)
    {
        ...
        if ($this->has8bitChars(substr($address, ++$pos)) && static::idnSupported()) {
            if (static::$REPLYTO != $kind) {
    ...
    {
        if (!in_array($kind, ['to', 'cc', 'bcc', static::$REPLYTO])) {
    ...
    public static function parseAddresses($addrstr, $useimap = true)
    {
        ...
        if ($useimap && function_exists('imap_rfc822_parse_adrlist')) {
            ...
            foreach ($list as $address) {
                ...
                if (strpos($address, '<') === false && static::validateAddress($address)) {
    ...
    public function preSend()
    {
        ...
        if (ini_get('mail.add_x_header') == 1 && 'mail' == $this->Mailer && stripos(PHP_OS, 'WIN') === 0 && ((version_compare(PHP_VERSION, '7.0.0', '>=') && version_compare(PHP_VERSION, '7.0.17', '<'))
            || (version_compare(PHP_VERSION, '7.1.0', '>=') && version_compare(PHP_VERSION, '7.1.3', '<')))
            ) {
                trigger_error(
    ...
    public function setLanguage($langcode = 'en', $lang_path = '')
    {
        ...
        $PHPMAILER_LANG = [
            ...
            static::$INV_ADDR => 'Invalid address: ',
    ...
    public function createHeader()
    {
        ...
        if ((
            static::$SENDEMAIL == $this->Mailer || static::$QMAIL == $this->Mailer || 'mail' == $this->Mailer
            )
            && count($this->bcc) > 0
            ) {
            ...
            if (count($this->ReplyTo) > 0) {
                $result .= $this->addrAppend(static::$REPLYTO, $this->ReplyTo);
            }
    ...
    public function createBody()
    {
        ...
        switch ($this->message_type) {
            ...
            case 'alt':
                $body .= $mimepre;
                $body .= $this->getBoundary($this->boundary[1], $altBodyCharSet, static::$TXT_PLAN, $altBodyEncoding);
                $body .= $this->encodeString($this->AltBody, $altBodyEncoding);
                $body .= static::$LE;
                $body .= $this->getBoundary($this->boundary[1], $bodyCharSet, static::$TXT_HTML, $bodyEncoding);
                $body .= $this->encodeString($this->Body, $bodyEncoding);
                $body .= static::$LE;
                if (!empty($this->Ical)) {
            ...
            case 'alt_inline':
                ...
                $body .= $this->textLine("\tboundary=\"" . $this->boundary[2] . '"');
                $body .= static::$LE;
                $body .= $this->getBoundary($this->boundary[2], $bodyCharSet, static::$TXT_HTML, $bodyEncoding);
                $body .= $this->encodeString($this->Body, $bodyEncoding);
                $body .= static::$LE;
                $body .= $this->attachAll('inline', $this->boundary[2]);
                $body .= static::$LE;
            ...
            case 'alt_attach':
                ...
                $body .= $this->textLine("\tboundary=\"" . $this->boundary[2] . '"');
                $body .= static::$LE;
                $body .= $this->getBoundary($this->boundary[2], $altBodyCharSet, static::$TXT_PLAN, $altBodyEncoding);
                $body .= $this->encodeString($this->AltBody, $altBodyEncoding);
                $body .= static::$LE;
                $body .= $this->getBoundary($this->boundary[2], $bodyCharSet, static::$TXT_HTML, $bodyEncoding);
                $body .= $this->encodeString($this->Body, $bodyEncoding);
            ...
            case 'alt_inline_attach':
                ...
                $body .= static::$LE;
                $body .= $this->getBoundary($this->boundary[3], $bodyCharSet, static::$TXT_HTML, $bodyEncoding);
                $body .= $this->encodeString($this->Body, $bodyEncoding);
                $body .= static::$LE;
        ...
        if (array_key_exists(2, $images)) {
            ...
            foreach ($images[2] as $imgindex => $url) {
                ...
                if (preg_match('#^data:(image/(?:jpe?g|gif|png));?(base64)?,(.+)#', $url, $match)) {
                    if (count($match) == 4 && 'base64' == $match[2]) {
    ...
    {
        $mime_html = static::$TXT_HTML;
        $mime_plan = static::$TXT_PLAN;
        $mime_stream = 'application/octet-stream';
        $mime_postscript = 'application/postscript';
        $mime_director = 'application/x-director';
        $mime_php = 'application/x-httpd-php';
        $mime_mpeg = 'audio/mpeg';
        $mime_aiff = 'audio/x-aiff';
        $mime_jpeg = 'image/jpeg';
        $mime_vmpeg = 'video/mpeg';
